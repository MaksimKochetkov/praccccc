...
import com.example.au_zad1.databinding.FragmentAddNoteBinding
...
class AddNoteFragment : Fragment(R.layout.fragment_add_note),MenuProvider {
    ...
    private fun saveNote(view: View) {
        ...
        if (noteTitle.isNotEmpty()) {
            ...
            Toast.makeText(addNoteView.context, "Заметка создана", Toast.LENGTH_SHORT).show()
            view.findNavController().popBackStack(R.id.homeFragment2, false)
        }
        else {
            Toast.makeText(addNoteView.context, "Укажите название заметки", Toast.LENGTH_SHORT).show()
        }
    }
